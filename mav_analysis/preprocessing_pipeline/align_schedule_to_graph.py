...
class ScheduleProcessor:
    ...
    def preprocess_schedules(
        ...
        # START: HANDLING TIME JUMPS OVER MIDNIGHT
        # Create a copy for processing midnight crossings
        time_df = schedules_df[
            ["elvira_id", "observation_sequence_id"] + schedule_time_cols
        ].copy()

        # Sort once at the beginning
        time_df = time_df.sort_values(["elvira_id", "observation_sequence_id"])

        # We'll iterate over each time column, but everything else is done vectorized
        for col in schedule_time_cols:
            # If the column is completely missing or mostly NaN, skip if needed
            # if time_df[col].isna().sum() > len(time_df) * 0.5:
            #     continue

            # 1) Compute time differences per group
            time_diffs = time_df.groupby("elvira_id")[col].diff()

            # 2) Identify negative jumps more than -1 hour
            #    (the 'notna()' check helps avoid false comparisons on NaN)
            jumps = time_diffs.notna() & (time_diffs < pd.Timedelta(hours=-1))

            # 3) Convert these detections to a day-shift that resets per group
            #    cumsum() on Boolean True/False will produce 0,1,2,... for each group
            day_shift = jumps.groupby(time_df["elvira_id"]).cumsum()

            # 4) Add the day shift to the original timestamps for non-null rows
            #    Only shift rows that actually have a timestamp
            not_null_mask = time_df[col].notna()
            time_df.loc[not_null_mask, col] += pd.to_timedelta(
                day_shift[not_null_mask], unit="D"
            )

        # Update original dataframe with corrected times
        for col in schedule_time_cols:
            schedules_df[col] = time_df[col]
        # END: HANDLING TIME JUMPS OVER MIDNIGHT
...
def align_schedule_to_graph(bidirected: bool = True) -> None:
    if bidirected:
        with open(
